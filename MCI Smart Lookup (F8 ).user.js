...
// @namespace    mci-tools
// @version      1.11
// @description  F8 on hover/selection: LinkedIn, Wake+Maps+Vexcel, Erie (WWW), NatGen. Tab title shows detected type. On-site automations run ONLY when armed by F8.
// @match        *://*/*
// @match        file://*/*
...
(function(){
  ...
  const CFG = {
    ...
    // NEW: On-site automations will ONLY run if armed by F8 within this TTL window.
    armedTTLms: 60000
  };
  ...
  // NEW: F8 arming gate (prevents auto-run when you browse normally)
  const K_ARMED   = "mci.f8.armed";
  const K_ARMEDTS = "mci.f8.armed.ts";

  function armAutomations(){
    try{
      sessionStorage.setItem(K_ARMED, "1");
      sessionStorage.setItem(K_ARMEDTS, String(Date.now()));
    }catch(_){}
  }
  function disarmAutomations(){
    try{
      sessionStorage.removeItem(K_ARMED);
      sessionStorage.removeItem(K_ARMEDTS);
    }catch(_){}
  }
  function isArmed(){
    try{
      if(sessionStorage.getItem(K_ARMED) !== "1") return false;
      const ts = parseInt(sessionStorage.getItem(K_ARMEDTS) || "0", 10);
      if(!ts) return false;
      if(Date.now() - ts > (CFG.armedTTLms || 60000)){
        disarmAutomations();
        return false;
      }
      return true;
  ...
  document.addEventListener("keydown",(e)=>{
    ...
    // NEW: Only after F8 do we allow any on-site automation to run
    armAutomations();
  ...
  /* ============ ON-SITE AUTOMATIONS (ONLY WHEN ARMED) ============ */
  ...
  if (location.hostname === "www.agentexchange.com" || location.hostname === "agentexchange.com") {
    (function erieRun(){
      // NEW gate: never run unless armed by F8
      if(!isArmed()) return;
      ...
      function finish(){
        ...
        disarmAutomations(); // NEW: one-shot
      }
